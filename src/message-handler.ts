...
import { defaultSleep } from "./player";
 ...
 */
export function extractGuideFromMessage(data: unknown): Record<string, any> | null {
  if (!data) return null;
...
export interface LiveClassCanvasRef {
  ...
  magnifyArea?: (area: Record<string, unknown>, options?: { isMobile?: boolean }) => Promise<void> | void;
  zoomObject?: (targetId: string | number) => Promise<void> | void;
  removeObjectWithId?: (id: string | number) => void;
  selectObject?: (target: { id: string | number }) => void;
...
export interface LiveClassMessageContext {
  ...
  /** 현재 활성화된 스크립트 */
  currentScript?: { id: number | string; [key: string]: unknown } | null;
  /** 스크립트 목록 (단위/유닛 목록) */
  scriptList?: Array<{
    scripts: Array<{ id: number | string; [key: string]: unknown }>;
    [key: string]: unknown;
  }>;
  /** 캔버스 제어 핸들러 또는 ref */
 ...
 */
export async function handleLiveClassMessage(
  data: unknown,
  context: LiveClassMessageContext,
  options?: {
    platform?: PlatformType;
  },
): Promise<LiveMessageResult> {
  const isMobile =
    (options?.platform ?? context.platform ?? (isMobileDevice() ? "mobile" : "desktop")) ===
    "mobile";
  ...
  switch (guide.action) {
    case "property": {
      if (guide.lines) {
        if (guide.lines.PREPS) context.setPrepMembers?.(guide.lines.PREPS);
        if (guide.lines.SYNCS) context.setSyncMembers?.(guide.lines.SYNCS);
        if (guide.lines.GROUPS) context.setGroupMembers?.(guide.lines.GROUPS);
      }
      break;
    }
