...
exports.forwardToPoint = exports.STEP_ORDER = void 0;
exports.defaultSleep = defaultSleep;
exports.getLeaderIdForCut = getLeaderIdForCut;
exports.findNearestCleanBefore = findNearestCleanBefore;
exports.fastforward = fastforward;
exports.playToPoint = playToPoint;
exports.runNext = runNext;
exports.runAt = runAt;
exports.STEP_ORDER = ["prologue", "main", "epilogue"];
 ...
 */
function getLeaderIdForCut(cut) {
 ...
 */
function findNearestCleanBefore(cuts, targetIdx) {
 ...
 */
async function fastforward(cutContents, targetGuide, callbacks) {
    ...
    for (let idx = startIdx; idx <= endIdx; idx++) {
        ...
        const targetStepIdx = exports.STEP_ORDER.indexOf(targetStep);
        const limit = idx === endIdx && targetStepIdx !== -1 ? targetStepIdx : exports.STEP_ORDER.indexOf("epilogue");
        for (let s = 0; s <= limit; s++) {
            const step = exports.STEP_ORDER[s];
            if (step === "prologue") {
        ...
        if (callbacks.runMagnifier) {
            const stepsToRun = exports.STEP_ORDER.slice(0, limit + 1);
...
exports.forwardToPoint = fastforward;
/**
 ...
 */
async function playToPoint(cutContents, targetGuide, lastGuide, callbacks) {
    const sleep = callbacks.sleepFn ?? defaultSleep;
    if (!cutContents || cutContents.length === 0) {
        const state = {
            currentIdx: targetGuide.studyIdx ?? 0,
    ...
    for (let idx = lastPointIdx; idx <= endIdx; idx++) {
        ...
        const lastStep = idx === lastPointIdx && lastGuide
            ...
            : "prologue";
        const startStepIdx = idx === lastPointIdx ? exports.STEP_ORDER.indexOf(lastStep) : 0;
        const targetStepIdx = exports.STEP_ORDER.indexOf(targetStep);
        const limit = idx === endIdx && targetStepIdx !== -1 ? targetStepIdx : exports.STEP_ORDER.indexOf("epilogue");
        ...
        if (callbacks.runMagnifier) {
            ...
            const stepsToRun = exports.STEP_ORDER.slice(startIdxSlice, limit + 1);
            await callbacks.runMagnifier({ idx, steps: stepsToRun });
            await sleep(600);
        }
    }
 ...
 * (기준: topic-portrait-live/PageClient.tsx)
 */
async function runNext(cutContents, currentIdx, currentStep, callbacks) {
    const sleep = callbacks.sleepFn ?? defaultSleep;
    if (!cutContents || cutContents.length === 0) {
 ...
 * (기준: topic-portrait-live/PageClient.tsx)
 */
async function runAt(cutContents, targetIdx, targetStep, callbacks) {
    const sleep = callbacks.sleepFn ?? defaultSleep;
